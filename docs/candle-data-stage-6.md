...
Ця гілка не створює випадкові тики та не використовує TradingView, Binance або Twelve Data. До підключення авторизованого read-only Pocket WebSocket API свічок може бути порожнім зі статусом `warming`.

Наступний мережевий адаптер повинен лише декодувати реальні Pocket-повідомлення та передавати нормалізовані тики у `MarketDataPipeline`. Відкриття угод до цього потоку не входить.
